...
namespace NuGetGallery
{
        ...
        : AppController
    {
        ...
        public virtual async Task<ActionResult> UndoPendingEdits(string id, string version)
        {
            ...
            {
                await _auditingService.SaveAuditRecord(new PackageAuditRecord(package, PackageAuditAction.UndoEdit));
        ...
        public virtual async Task<ActionResult> Edit(string id, string version, EditPackageRequest formData, string returnUrl)
        {
            ...
            if (formData.Edit != null)
            {
                try
                {
                    ...
                    var simulatedPackage = formData.Edit.Simulate(package);

                    await _auditingService.SaveAuditRecord(new PackageAuditRecord(simulatedPackage, PackageAuditAction.Edit));
                }
        ...
        public virtual async Task<ActionResult> VerifyPackage(VerifyPackageRequest formData)
        {
            ...
            using (Stream uploadFile = await _uploadFileService.GetUploadFileAsync(currentUser.Key))
            {
                ...
                await _auditingService.SaveAuditRecord(new PackageAuditRecord(package, PackageAuditAction.Created, "Package created from web interface."));
